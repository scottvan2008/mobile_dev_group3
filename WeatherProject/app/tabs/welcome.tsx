...
const Welcome: React.FC = () => {
  ...
  const fetchSavedLocations = async (uid: string) => {
    try {
      ...
      if (error) Alert.alert("Error", "Unable to fetch your saved locations.");
        ...
        const locs = (data || []).map((loc: any) => ({ ...loc, isLoadingWeather: true }));
        ...
        batch.map((loc) =>
          fetchWeatherForLocation(loc.latitude, loc.longitude)
            .then((weather) => {
              ...
              if (idx !== -1) updated[idx] = { ...updated[idx], weather, isLoadingWeather: false };
            })
            .catch((err) => {
              console.error(err);
              const idx = updated.findIndex((l) => l.id === loc.id);
              if (idx !== -1) updated[idx] = { ...updated[idx], isLoadingWeather: false };
            })
        )
      );
  ...
  const fetchWeatherForLocation = async (lat: number, lon: number): Promise<WeatherData | null> => {
    try {
      ...
      return { temperature: data.current.temperature_2m, weathercode: data.current.weathercode, is_day: data.current.is_day };
    } catch (e) {
      console.error(e);
      return null;
    }
  ...
  const toggleAddLocation = () => {
    setSearchQuery("");
    setSearchResults([]);
    Animated.timing(addLocAnim, { toValue: showAddLocation ? 0 : 1, duration: 300, useNativeDriver: false }).start();
    setShowAddLocation(!showAddLocation);
  };

  const toggleMap = (loc?: SavedLocation) => {
    setSelectedLocation(loc || null);
    Animated.timing(mapAnim, { toValue: showMap ? 0 : 1, duration: 300, useNativeDriver: false }).start();
    setShowMap(!showMap);
  };
  ...
  const searchLocations = async (query: string) => {
    ...
    try {
      ...
      const data = await res.json();
      setSearchResults(
        data.results ? data.results.map((r: any) => ({ name: r.name, country: r.country, latitude: r.latitude, longitude: r.longitude })) : []
      );
    } catch (e) {
      console.error(e);
      setSearchResults([]);
  ...
  const saveLocation = async (loc: SearchResult) => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("saved_locations")
        .insert([{ user_id: userId, name: locName, latitude: loc.latitude, longitude: loc.longitude }])
        .select();
  ...
  const getCurrentLocation = async () => {
    try {
      ...
      if (rev.length && userId) {
        const address = rev[0];
        const locName = [address.city, address.region, address.country].filter(Boolean).join(", ");
        const { error } = await supabase
          .from("saved_locations")
          .insert([{ user_id: userId, name: locName || "Current Location", latitude, longitude }])
          .select();
  ...
  const getWeatherInfo = (code: number, isDay = 1): WeatherInfo => {
    const map: { [key: number]: WeatherInfo } = {
      0: { description: "Clear sky", icon: isDay ? "weather-sunny" : "weather-night" },
      1: { description: "Mainly clear", icon: isDay ? "weather-partly-cloudy" : "weather-night-partly-cloudy" },
      2: { description: "Partly cloudy", icon: isDay ? "weather-partly-cloudy" : "weather-night-partly-cloudy" },
      ...
      57: { description: "Heavy freezing drizzle", icon: "weather-snowy-rainy" },
      ...
      96: { description: "Thunderstorm with light hail", icon: "weather-lightning-rainy" },
      99: { description: "Thunderstorm with heavy hail", icon: "weather-hail" },
    };
    return map[code] || { description: "Unknown", icon: "weather-cloudy" };
  };

  const formatTemperature = (c: number) => `${Math.round(c)}°C`;
  const addLocPanelHeight = addLocAnim.interpolate({ inputRange: [0, 1], outputRange: [0, 350] });
  const mapPanelHeight = mapAnim.interpolate({ inputRange: [0, 1], outputRange: [0, Dimensions.get("window").height * 0.7] });
  const formatDate = (d: string) =>
    new Date(d).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });

  const renderLocationItem = ({ item }: { item: SavedLocation }) => {
    const info = item.weather ? getWeatherInfo(item.weather.weathercode, item.weather.is_day) : null;
    return (
      <View style={styles.locationCard}>
        <View style={styles.locationInfo}>
          <Text style={styles.locationName}>{item.name}</Text>
          <Text style={styles.locationDate}>Saved on {formatDate(item.created_at)}</Text>
        </View>
          ...
          {item.isLoadingWeather ? (
            ...
            <>
              <View style={styles.weatherIconContainer}>
                <Icon name={info?.icon || "weather-cloudy"} size={24} color="#4da0b0" />
                <Text style={styles.weatherDescription}>{info?.description}</Text>
              </View>
        ...
        <View style={styles.locationActions}>
          <TouchableOpacity style={styles.locationAction} onPress={() => toggleMap(item)}>
            <Icon name="map" size={20} color="#4da0b0" />
          </TouchableOpacity>
          <TouchableOpacity style={styles.locationAction} onPress={() => viewWeather(item)}>
            <Icon name="weather-partly-cloudy" size={20} color="#4da0b0" />
          </TouchableOpacity>
          <TouchableOpacity
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.container}>
        <LinearGradient colors={["#4da0b0", "#d39d38"]} style={styles.gradientBackground}>
          ...
          <View style={styles.content}>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity style={styles.addButton} onPress={toggleAddLocation}>
          ...
          <Animated.View style={[styles.addLocationPanel, { height: addLocPanelHeight }]}>
            ...
            {isSearching ? (
              <ActivityIndicator style={styles.searchLoading} color="#3182CE" />
            ) : (
              <FlatList
                data={searchResults}
                ...
                ListEmptyComponent={
                  searchQuery.length > 0 ? (
                    ...
                    <Text style={styles.searchPromptText}>Search for a city to add to your saved locations.</Text>
                  )
                }
              />
            )}
              ...
              >
                <Marker coordinate={{ latitude: selectedLocation.latitude, longitude: selectedLocation.longitude }} title={selectedLocation.name}>
                  <View style={styles.markerContainer}>
                    <Icon name="map-marker" size={24} color="#3182CE" />
                  </View>
                </Marker>
              </MapView>
            )}
...
const styles = StyleSheet.create({
  ...
  loadingContainer: { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#EBF8FF" },
  ...
  logoutButton: { backgroundColor: "rgba(0,0,0,0.2)", padding: 10, borderRadius: 20 },
  ...
  addButton: { backgroundColor: "rgba(0,0,0,0.2)", padding: 8, borderRadius: 16 },
  currentLocationButton: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.2)", borderRadius: 10, paddingVertical: 12, paddingHorizontal: 16, marginBottom: 15 },
  currentLocationText: { color: "white", fontSize: 16, fontWeight: "500", marginLeft: 8 },
  locationsLoading: { marginTop: 20, alignSelf: "center" },
  emptyState: { alignItems: "center", justifyContent: "center", backgroundColor: "rgba(0,0,0,0.1)", borderRadius: 12, padding: 30, marginTop: 10 },
  emptyStateText: { color: "white", fontSize: 16, fontWeight: "500", marginTop: 15, textAlign: "center" },
  emptyStateSubtext: { color: "rgba(255,255,255,0.7)", fontSize: 14, marginTop: 5, textAlign: "center" },
  locationsList: { paddingBottom: 20, flexGrow: 1 },
  locationCard: { backgroundColor: "white", borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 2 },
  locationInfo: { marginBottom: 10 },
  locationName: { fontSize: 16, fontWeight: "600", color: "#2D3748", marginBottom: 4 },
  locationDate: { fontSize: 12, color: "#718096" },
  weatherInfo: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 10, borderTopWidth: 1, borderBottomWidth: 1, borderColor: "#EDF2F7", marginBottom: 10 },
  ...
  appInfoSubtext: { color: "rgba(255,255,255,0.6)", fontSize: 12, marginTop: 4 },
  addLocationPanel: { position: "absolute", top: 0, left: 0, right: 0, backgroundColor: "white", borderBottomLeftRadius: 16, borderBottomRightRadius: 16, overflow: "hidden", zIndex: 10, elevation: 5, shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 3.84 },
  ...
  searchPromptText: { textAlign: "center", padding: 20, color: "#666", fontStyle: "italic" },
  ...
  map: { flex: 1 },
  markerContainer: { backgroundColor: "white", borderRadius: 20, padding: 6, borderWidth: 2, borderColor: "#3182CE" },
  mapActions: { padding: 16, borderTopWidth: 1, borderTopColor: "#eee" },
  mapActionButton: { backgroundColor: "#3182CE", borderRadius: 8, paddingVertical: 12, paddingHorizontal: 16, flexDirection: "row", alignItems: "center", justifyContent: "center" },
  mapActionText: { color: "white", fontSize: 16, fontWeight: "600", marginLeft: 8 },
});
